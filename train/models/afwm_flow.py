...
def flow_conv(flow,weight):
    flow_x, flow_y = torch.split(flow, 1, dim=1)
    delta_x = F.conv2d(flow_x, weight)
    delta_y = F.conv2d(flow_y, weight)
...
class AFlowNet(nn.Module):
    def __init__(self, num_pyramid, fpn_dim=256):
        ...
        for i in range(num_pyramid):
            ...
            netRefine_bg = torch.nn.Sequential(
                torch.nn.Conv2d(2 * fpn_dim, out_channels=128, kernel_size=3, stride=1, padding=1),
                torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
                torch.nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1),
                torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            ...
            style_block_bg = StyledConvBlock(256, 49, latent_dim=256,
                                          padding=padding_type, actvn=actvn,
                                          normalize_affine_output=normalize_mlp,
                                          modulated_conv=modulated_conv)
            ...
            style_Fout_block = Styled_F_ConvBlock(49, 2, latent_dim=256,
                                                  padding=padding_type, actvn=actvn,
                                                  normalize_affine_output=normalize_mlp,
                                                  modulated_conv=modulated_conv)

            self.netRefine.append(netRefine_layer)
            self.netRefine_bg.append(netRefine_bg)
            self.netStyle.append(style_block)
            self.netStyle_bg.append(style_block_bg)
            self.netF.append(style_F_block)
            self.netF_out.append(style_Fout_block)

        self.netRefine = nn.ModuleList(self.netRefine)
        self.netRefine_bg = nn.ModuleList(self.netRefine_bg)
        self.netStyle = nn.ModuleList(self.netStyle)
        self.netStyle_bg = nn.ModuleList(self.netStyle_bg)
        self.netF = nn.ModuleList(self.netF)
        self.netF_out = nn.ModuleList(self.netF_out)
    ...
    def forward(self, x, x_edge, x_warps, x_conds, occ_mask, warp_feature=True):
        last_flow = None
        last_flow_bg = None
        last_flow_all = []
        last_flow_all_bg = []
        ...
        for i in range(len(x_warps)):
            ...
            cur_occ_mask = F.interpolate(occ_mask, scale_factor=0.5 ** (4 - i), mode='bilinear')
            cur_non_occ = 1 - cur_occ_mask
            ...
            if last_flow is not None and warp_feature:
                ...
                x_warp_after_bg = F.grid_sample(x_warp, last_flow_bg.detach().permute(0, 2, 3, 1),
                                             mode='bilinear', padding_mode='border')
            else:
                x_warp_after = x_warp
                x_warp_after_bg = x_warp
            # Global warping using style modulation
            stylemap = self.netStyle[i](x_warp_after, style)
            stylemap_bg = self.netStyle_bg[i](x_warp_after_bg,style)
            flow = self.netF[i](stylemap, style)
            flow_bg = self.netF_out[i](stylemap_bg, style)
            # Store delta flow for TV loss
            delta_list.append(flow)
            delta_list_bg.append(flow_bg)
            # apply offset
            flow = apply_offset(flow)
            flow_bg = apply_offset(flow_bg)
            if last_flow is not None:
                flow = F.grid_sample(last_flow, flow, mode='bilinear', padding_mode='border')
                flow_bg = F.grid_sample(last_flow_bg, flow_bg, mode='bilinear', padding_mode='border')
            else:
                flow = flow.permute(0, 3, 1, 2)
                flow_bg = flow_bg.permute(0, 3, 1, 2)
            # Get refine flow
            last_flow = flow
            last_flow_bg = flow_bg
            x_warp_cur = x_warp
            x_warp = F.grid_sample(x_warp, flow.permute(0, 2, 3, 1), mode='bilinear', padding_mode='border')
            x_warp_bg = F.grid_sample(x_warp_cur, flow_bg.permute(0, 2, 3, 1), mode='bilinear', padding_mode='border')
            concat = torch.cat([x_warp, x_cond], 1)
            concat_bg = torch.cat([x_warp_bg, x_cond], 1)
            flow = self.netRefine[i](concat)
            flow_bg = self.netRefine_bg[i](concat_bg)
            delta_list.append(flow)
            delta_list_bg.append(flow_bg)
            flow = apply_offset(flow)
            flow_bg = apply_offset(flow_bg)
            # Warp previous flow with latest flow
            flow = F.grid_sample(last_flow, flow, mode='bilinear', padding_mode='border')
            flow_bg = F.grid_sample(last_flow_bg, flow_bg, mode='bilinear', padding_mode='border')
            last_flow = F.interpolate(flow, scale_factor=2, mode='bilinear')
            last_flow_bg = F.interpolate(flow_bg, scale_factor=2, mode='bilinear')
            ...
            cur_x_warp_bg = F.grid_sample(cur_x, last_flow_bg.permute(0, 2, 3, 1), mode='bilinear',padding_mode='border')
            x_all_bg.append(cur_x_warp_bg)
            cur_x_edge = F.interpolate(x_edge, scale_factor=0.5 ** (len(x_warps) - 1 - i), mode='bilinear')
           
            last_flow_all.append(last_flow)
            last_flow_all_bg.append(last_flow_bg)
            cur_x_warp_edge = F.grid_sample(cur_x_edge, last_flow.permute(0, 2, 3, 1), mode='bilinear',
                                            padding_mode='zeros')
            cur_x_warp_edge_bg = F.grid_sample(cur_x_edge, last_flow_bg.permute(0, 2, 3, 1), mode='bilinear',
                                            padding_mode='zeros')
            final_edge = (cur_x_warp_edge * cur_occ_mask)+(cur_x_warp_edge_bg*cur_non_occ)
            x_edge_all.append(final_edge)
            delta_x , delta_y = flow_conv(last_flow,self.weight)
            delta_x_bg , delta_y_bg = flow_conv(last_flow_bg,self.weight)
            delta_x_all.append(delta_x)
            delta_y_all.append(delta_y)
            delta_x_all_bg.append(delta_x_bg)
            delta_y_all_bg.append(delta_y_bg)

        x_warp = F.grid_sample(x, last_flow.permute(0, 2, 3, 1),
                               mode='bilinear', padding_mode='border')
        x_warp_bg = F.grid_sample(x, last_flow_bg.permute(0, 2, 3, 1),
                               mode='bilinear', padding_mode='border')
        final_warp = (x_warp*occ_mask)+(x_warp_bg*(1-occ_mask))
        return final_warp, last_flow,last_flow_bg, cond_fea_all, last_flow_all,last_flow_all_bg, delta_list,delta_list_bg, x_all, x_all_bg, x_edge_all, delta_x_all, delta_y_all,delta_x_all_bg,delta_y_all_bg


class AFWM_Flow(nn.Module):

    def __init__(self, opt, input_nc):
        super(AFWM_Flow, self).__init__()
    ...
    def forward(self, cond_input, image_input, image_edge, occ_mask):
        ...
        final_warp, last_flow,last_flow_bg, cond_fea_all, last_flow_all,last_flow_all_bg, delta_list,delta_list_bg, x_all, x_all_bg, x_edge_all, delta_x_all, delta_y_all,delta_x_all_bg,delta_y_all_bg = self.aflow_net(
            image_input, image_edge, image_pyramids, cond_pyramids, occ_mask)
