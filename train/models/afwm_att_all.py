...
class SelfAttentionFlow(nn.Module):
    def __init__(self, in_channels,k):
        ...
        self.query = nn.Conv2d(in_channels, kernel_size=1)
    ...
    def forward(self, x):
        B, C, H, W = x.size()
        x = x.view(B,self.k,2,-1)
        query = self.query(x).view(B, self.k,-1, H*W) # (B, k, 2//8, H*W)
        key = self.key(x).view(B, self.k,-1, H*W) # (B, k, 2//8, H*W)
        value = self.value(x).view(B, self.k, 2, -1) # (B, k, 2, H*W)

        attn = torch.einsum('bklj,bkmj->bkj', query, key) # (B, k, H*W)
        attn = attn / (C // 8) ** 0.5 # scaling factor
        attn = torch.softmax(attn, dim=1)
        out = torch.einsum('bkj,bkmj->bjm',attn,value) # (B, 2, H*W)
...
class AFlowNet(nn.Module):
    def __init__(self, num_pyramid, fpn_dim=256, k=4,device="cuda"):
        ...
        for i in range(num_pyramid):
            ...
            style_F_block = Styled_F_ConvBlock(49,2*k, latent_dim=256,
                                               ...
                                               normalize_affine_output=normalize_mlp,
                                               modulated_conv=modulated_conv)
            self.global_att.append(SelfAttentionFlow(k,k).to(device))
    ...
    def forward(self, x, x_edge, x_warps, x_conds, warp_feature=True,k=4):
        ...
        for i in range(len(x_warps)):
            ...
            flow_map = self.netF[i](stylemap, style)
            print(flow_map.shape)
            flow = self.global_att[i](flow_map)
            print(flow.shape)
            ...
            flow_map = flow_and_maps[:, :k * 2, :, :]
            k_att = F.softmax(flow_and_maps[:, k * 2:, :, :],dim=1)
            print(k_att.shape)
            # Get the index of the flow map with the highest probability for each position
            attention_map_max_idx = k_att.argmax(dim=1)
            flow = flow_map.gather(1, attention_map_max_idx.unsqueeze(1).repeat(1,2,1,1))  # use the indices to
            #print(flow.shape)
            delta_list.append(flow)
            flow = apply_offset(flow)
            flow = F.grid_sample(last_flow, flow, mode='bilinear', padding_mode='border')
